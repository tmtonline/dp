...
<div class="row">
	<div class="span12" style="border-bottom:1px solid #f5f5f5;">
		<div class="row">
			...
			<div class="span8">
				<?php echo form_open($this->config->item('admin_folder').'/leaves/index', 'class="form-inline" style="float:right"');?>
					<fieldset>												
			...
			<tr>
				...
				<td>
					...
					<?php if($leave->status !== "Cancelled"): ?>						
							<a class="btn" href="<?php echo  site_url($this->config->item('admin_folder').'/leaves/form/'.$leave->id);?>"><i class="icon-pencil"></i>  <?php echo lang('edit');?></a>													
					<?php endif; ?>
							<a class="btn btn-danger" href="<?php echo  site_url($this->config->item('admin_folder').'/leaves/delete/'.$leave->id);?>" onclick="return areyousure();"><i class="icon-trash icon-white"></i> <?php echo lang('delete');?></a>
					</span>
				</td>
			</tr>
	<?php endforeach; ?>
